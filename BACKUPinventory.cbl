...
PROCEDURE DIVISION.
000-FULFILL-ORDERS.
	PERFORM INIT-INV-TABLE VARYING NUM-IN FROM 1 BY 1 UNTIL NUM-IN IS GREATER THAN 24.
	OPEN INPUT INV-FILE.
	PERFORM 010-IMPORT-INVENTORY UNTIL INV-EOF-SWITCH IS EQUAL TO "Y".
	CLOSE INV-FILE.

	DISPLAY WS-INVENTORY-TABLE.
	
	PERFORM INIT-CUST-TABLE VARYING NUM-IN FROM 1 BY 1 UNTIL NUM-IN IS GREATER THAN 10.
	OPEN INPUT CUST-FILE.
	PERFORM 020-IMPORT-CUSTOMERS.
	CLOSE CUST-FILE.

	DISPLAY WS-CUST-TABLE.
...
INIT-INV-TABLE.
	MOVE ZEROS TO WS-INV-ITEM-NO(NUM-IN).
	MOVE SPACES TO WS-INV-ITEM-NAME(NUM-IN).
	MOVE ZEROS TO WS-INV-QTY(NUM-IN).
	MOVE ZEROS TO WS-INV-REORDER-PT(NUM-IN).
	MOVE ZEROS TO WS-INV-PRICE(NUM-IN).

INIT-CUST-TABLE.
	MOVE ZEROS TO WS-CUST-NO(NUM-IN).
	MOVE SPACES TO WS-CUST-NAME(NUM-IN).
	MOVE SPACES TO WS-CUST-STREET(NUM-IN).
	MOVE SPACES TO WS-CUST-CITY(NUM-IN).
	MOVE SPACES TO WS-CUST-REGION(NUM-IN).
	MOVE ZEROS TO WS-CUST-PAST-DUE(NUM-IN).
